...
public class RunExperimentHarmonySUB {

	static String onto1 = "302";
	...
	final static String storePath = "./files/_PHD_EVALUATION/OAEI2011/HARMONY/SUBSUMPTION/"+ onto1+onto2;
	...
	public static void main(String[] args) throws AlignmentException, URISyntaxException, IOException, OWLOntologyCreationException, JWNLException {
		...
		if (ontologyProfilingScores.get("cf") > 0.2) {
			runCompoundMatcher(ontologyProfilingScores.get("cf"));
		}

		if (ontologyProfilingScores.get("sa") > 0.2) {
		...
		if (ontologyProfilingScores.get("hrw") > 0.2) {
			runLexicalSubsumptionMatcher(ontologyProfilingScores.get("hrw"));
		}

		if (ontologyProfilingScores.get("dte") > 0.2) {
		...
		String alignmentFileName = "./files/_PHD_EVALUATION/OAEI2011/HARMONY/SUBSUMPTION/ComputedHarmonyAlignment_" + onto1+onto2 + ".rdf";
		...
		System.out.println("storedHarmonyAlignment contains the following relations");
		for (Cell c : storedHarmonyAlignment) {
